...
=======
- [Operational Memory Gates](https://github.com/NiraNexus/operational-memory-gates) - MIT-licensed specification and reference implementation of 20 mechanical pre-code gates that refuse (non-zero exit) and 1 that advises. Check 21 never increments $fail. Each gate is documented as either a production fire made mechanical or a discipline made mechanical; the reference script is hardcoded to the author's stack by design. Includes a clean-room model roster audit that verifies model IDs against the live OpenRouter directory.
